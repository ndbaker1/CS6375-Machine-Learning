...
for n in $(seq $1); do
    for n in 2 5 10 15 20; do
        ...
        python -c "print( str($n) + ',' + str($(stat -c %s ./images/Koala.jpg) / $(stat -c %s ./images/Koala-$n.jpg)) )" >> Koala.csv
    done

    for n in 2 5 10 15 20; do
        java -cp classes KMeans ./images/Penguins.jpg $n ./images/Penguins-$n.jpg
        python -c "print( str($n) + ',' + str($(stat -c %s ./images/Penguins.jpg) / $(stat -c %s ./images/Penguins-$n.jpg)) )" >> Penguins.csv
    done
done